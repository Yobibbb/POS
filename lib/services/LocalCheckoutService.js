...
export class LocalCheckoutService {
  ...
  async uploadProducts(products) {
    ...
    try {
      const res = await fetch(`${this.serverUrl}/api/products/upload`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (err.message.includes('Server error') || err.message.includes('error')) {
        throw err;
      }
      throw new Error(
        `Cannot reach local server at ${this.serverUrl}. Make sure the server is running.`
      );
    }
  }
